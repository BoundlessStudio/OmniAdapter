...
public class OpenAiClientTests
{
    ...
    [TestMethod]
    [DataRow(ImageResponseFormat.B64_Json)]
    [DataRow(ImageResponseFormat.Url)]
    public async Task GenerateImageUrl(ImageResponseFormat imageResponseFormat)
    {
        var messages = new List<InputMessage> {
            new SystemMessage("You are a helpful assistant."),
            new UserMessage("Generate a cat image"),
    };
        ...
        var response = await _client.GenerateImagesAsync(request);

        Assert.IsNotNull(response);
        Assert.IsNotNull(response.Results);
        Assert.AreEqual(1, response.Results.Count);

        Console.WriteLine($"Image: {response.Results[0]} ");
    }
}
